...
func remove_card(index: int) -> Card3D:
	...
	GameData.current_battle.opponent_player.district_cards_in_hand.erase(removed_card.resource)
	return removed_card
...
func update_game_data() -> void:
	...
	GameData.current_battle.opponent_player.district_cards_in_hand = new_hand_data
